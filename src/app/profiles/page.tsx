...
export default function ProfilesPage() {
  const profiles = [
    { name: "Janandra", slug: "janandra", role: "Frontend Developer" },
    { name: "Akmal", slug: "akmal", role: "Backend Developer" },
    { name: "Farhan", slug: "farhan", role: "UI/UX Designer" },
  ];
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {profiles.map((p) => (
          <div key={p.slug} className="bg-bg-secondary border border-border p-8 rounded-2xl flex flex-col items-center text-center">
            ...
            <Badge variant="orange" className="mt-2 mb-6">{p.role}</Badge>
            <Link href={`/profiles/${p.slug}`} className="font-inter text-sm text-accent-teal font-semibold hover:underline">
              Lihat Jurnal →
            </Link>
          </div>
        ))}
